...
const ProductDetail = () => {
    ...
    return (
        <aside
            className={`${context.isProductDetailOpen ? 'flex' : 'hidden'} product-detail flex-col fixed right-0 top-0 border border-black rounded-lg bg-white shadow-2xl mt-20`}>
            <div className='flex justify-between items-center p-6'>
                <h2 className='font-medium text-xl'>Detalle del producto</h2>
                <div>
                    <XMarkIcon className="size-8 cursor-pointer text-blue-500 bg-white rounded-full border border-gray-300 shadow hover:bg-gray-200 transition p-1" onClick={() => context.closeProductDetail()} />
                </div>
            </div>
